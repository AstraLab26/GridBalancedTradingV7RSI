//+------------------------------------------------------------------+
//|                    GridBalancedTradingV7RSI.mq5                  |
//|         EA Grid Trading với bộ lọc RSI và cân bằng lưới tự động  |
//+------------------------------------------------------------------+
#property copyright "Grid Balanced Trading"
#property version   "7.0"
#property description "Grid Trading EA với bộ lọc RSI - EA chỉ khởi động khi RSI đủ điều kiện, lấy giá tại thời điểm đó làm gốc để tạo lưới"
...
enum ENUM_RSI_MODE
{
   RSI_MODE_OFF = 0,                    // Tắt bộ lọc RSI
   RSI_MODE_LEVEL = 1,                  // Chế độ trên/dưới mức (đặt lệnh khi RSI > mức trên hoặc RSI < mức dưới)
   RSI_MODE_CROSSOVER = 2,              // Chế độ cắt lên/xuống mức (đặt lệnh khi RSI cắt từ dưới lên mức trên hoặc cắt từ trên xuống mức dưới)
   RSI_MODE_BOTH = 3                    // Cả 2 chế độ (trên/dưới mức hoặc cắt lên/xuống)
};

//--- Input parameters - Bộ lọc RSI
input group "=== BỘ LỌC RSI ==="
input ENUM_RSI_MODE RSIMode = RSI_MODE_OFF;      // Chế độ RSI (0=Tắt, 1=Trên/dưới mức, 2=Cắt lên/xuống, 3=Cả 2)
input int RSIPeriod = 14;                       // Chu kỳ RSI
input double RSIUpperLevel = 70.0;              // Mức RSI trên
input double RSILowerLevel = 30.0;              // Mức RSI dưới
...
input string CommentOrder = "Grid Balanced V7RSI"; // Comment cho lệnh
...
int rsiHandle = INVALID_HANDLE;                 // Handle cho RSI indicator
double previousRSI = -1;                        // Giá trị RSI trước đó để phát hiện crossover
bool rsiGridInitialized = false;                // Flag đánh dấu lưới đã được khởi tạo khi đủ điều kiện RSI
...
int OnInit()
{
   ...
   Print("========================================");
   Print("Grid Balanced Trading EA V7 RSI đã khởi động");
   ...
   Print("--- Bộ lọc RSI ---");
   if(RSIMode != RSI_MODE_OFF)
   {
      // Khởi tạo RSI indicator
      rsiHandle = iRSI(_Symbol, PERIOD_CURRENT, RSIPeriod, PRICE_CLOSE);
      if(rsiHandle == INVALID_HANDLE)
      {
         Print("LỖI: Không thể tạo RSI indicator! Error: ", GetLastError());
         return(INIT_FAILED);
      }
      // Khởi tạo giá trị RSI trước đó (lấy giá trị hiện tại)
      previousRSI = GetRSIValue();
      if(previousRSI == -1)
      {
         previousRSI = -1; // Nếu lỗi thì để -1, sẽ coi như chưa có giá trị trước đó
      }
      Print("Bộ lọc RSI: ON | Chu kỳ: ", RSIPeriod, " | Mức trên: ", RSIUpperLevel, " | Mức dưới: ", RSILowerLevel);
      Print("  → Chế độ: ", EnumToString(RSIMode));
      if(RSIMode == RSI_MODE_LEVEL || RSIMode == RSI_MODE_BOTH)
      {
         Print("     ✓ Chế độ RSI trên/dưới mức: ON");
         Print("       - Đặt lệnh khi RSI > ", RSIUpperLevel, " hoặc RSI < ", RSILowerLevel);
      }
      if(RSIMode == RSI_MODE_CROSSOVER || RSIMode == RSI_MODE_BOTH)
      {
         Print("     ✓ Chế độ RSI cắt lên/xuống mức: ON");
         Print("       - Đặt lệnh khi RSI cắt từ dưới lên ", RSIUpperLevel, " (RSI hiện tại > ", RSIUpperLevel, " và RSI trước đó <= ", RSIUpperLevel, ")");
         Print("       - Đặt lệnh khi RSI cắt từ trên xuống ", RSILowerLevel, " (RSI hiện tại < ", RSILowerLevel, " và RSI trước đó >= ", RSILowerLevel, ")");
      }
      if(previousRSI != -1)
         Print("  → Giá trị RSI ban đầu: ", previousRSI);
      Print("  → EA sẽ chờ điều kiện RSI để khởi động lưới");
      rsiGridInitialized = false; // Chưa khởi tạo lưới, chờ điều kiện RSI
   }
   else
   {
      Print("Bộ lọc RSI: OFF");
      rsiGridInitialized = true; // Không dùng RSI, khởi tạo lưới ngay
   }
   Print("========================================");
...
void OnDeinit(const int reason)
{
   ...
   Print("Grid Balanced Trading EA V7 RSI đã dừng. Reason: ", reason);
}
...
void OnTick()
{
   ...
   // Kiểm tra RSI và khởi tạo lưới nếu chưa khởi tạo
   if(RSIMode != RSI_MODE_OFF && !rsiGridInitialized)
   {
      CheckRSIAndInitializeGrid();
   }
   
   // Chỉ quản lý lệnh khi lưới đã được khởi tạo và không ở chế độ Trading Stop
   if(rsiGridInitialized && !isTradingStopActive)
   {
      ManageGridOrders();
   }
   else if(RSIMode != RSI_MODE_OFF && !rsiGridInitialized)
   {
      // Log mỗi 100 tick để thông báo đang chờ điều kiện RSI
      static int waitTickCount = 0;
      waitTickCount++;
      if(waitTickCount % 100 == 0)
      {
         double currentRSI = GetRSIValue();
         if(currentRSI != -1)
            Print("⏳ Đang chờ điều kiện RSI để khởi động lưới... (RSI hiện tại: ", DoubleToString(currentRSI, 2), ")");
      }
   }
...
void ResetEA(string resetReason = "Thủ công")
{
   ...
   // Reset giá trị RSI trước đó và flag khởi tạo lưới
   if(RSIMode != RSI_MODE_OFF && rsiHandle != INVALID_HANDLE)
   {
      previousRSI = GetRSIValue();
      if(previousRSI == -1)
      {
         previousRSI = -1; // Nếu lỗi thì để -1
      }
      rsiGridInitialized = false; // Reset flag, chờ điều kiện RSI mới
   }
   else
   {
      rsiGridInitialized = true; // Không dùng RSI, khởi tạo lưới ngay
...
//+------------------------------------------------------------------+
//| Kiểm tra RSI và khởi tạo lưới khi đủ điều kiện                 |
//+------------------------------------------------------------------+
void CheckRSIAndInitializeGrid()
{
   if(RSIMode == RSI_MODE_OFF)
   {
      // Nếu không dùng RSI, khởi tạo lưới ngay
      rsiGridInitialized = true;
      return;
   }
   
   double rsiValue = GetRSIValue();
   if(rsiValue == -1) // Lỗi lấy giá trị RSI
   {
   ...
   // Chế độ 1: RSI trên/dưới mức (chỉ cần RSI > mức trên hoặc RSI < mức dưới)
   if(RSIMode == RSI_MODE_LEVEL || RSIMode == RSI_MODE_BOTH)
   {
      if(rsiValue > RSIUpperLevel)
      {
         canInitialize = true;
         reason = "RSI trên mức " + DoubleToString(RSIUpperLevel, 1) + " (RSI: " + DoubleToString(rsiValue, 2) + ")";
      }
      else if(rsiValue < RSILowerLevel)
      {
         canInitialize = true;
         reason = "RSI dưới mức " + DoubleToString(RSILowerLevel, 1) + " (RSI: " + DoubleToString(rsiValue, 2) + ")";
      }
   }
   
   // Chế độ 2: RSI cắt lên/xuống mức (cần có crossover)
   if(RSIMode == RSI_MODE_CROSSOVER || RSIMode == RSI_MODE_BOTH)
   {
      // Chỉ kiểm tra crossover nếu đã có giá trị RSI trước đó (không phải lần đầu)
      if(previousRSI != -1)
      {
         // Cắt từ dưới lên mức trên: RSI hiện tại > mức trên VÀ RSI trước đó <= mức trên
         if(rsiValue > RSIUpperLevel && previousRSI <= RSIUpperLevel)
         {
            canInitialize = true;
            reason = "RSI cắt từ dưới lên " + DoubleToString(RSIUpperLevel, 1) + " (RSI: " + DoubleToString(previousRSI, 2) + " -> " + DoubleToString(rsiValue, 2) + ")";
         }
         // Cắt từ trên xuống mức dưới: RSI hiện tại < mức dưới VÀ RSI trước đó >= mức dưới
         else if(rsiValue < RSILowerLevel && previousRSI >= RSILowerLevel)
         {
            canInitialize = true;
            reason = "RSI cắt từ trên xuống " + DoubleToString(RSILowerLevel, 1) + " (RSI: " + DoubleToString(previousRSI, 2) + " -> " + DoubleToString(rsiValue, 2) + ")";
         }
      }
   }
   
   // Cập nhật giá trị RSI trước đó (luôn cập nhật để theo dõi, đặc biệt cho chế độ crossover)
   previousRSI = rsiValue;
   
   // Nếu đủ điều kiện, khởi tạo lưới
   if(canInitialize)
   {
      Print("========================================");
      Print("✓ ĐỦ ĐIỀU KIỆN RSI - KHỞI ĐỘNG LƯỚI");
      Print("Lý do: ", reason);
      
      // Lấy giá hiện tại làm giá cơ sở mới
      double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
      double oldBasePrice = basePrice;
      basePrice = currentPrice;
      
      Print("Giá cơ sở cũ: ", oldBasePrice);
      Print("Giá cơ sở mới (tại thời điểm RSI đủ điều kiện): ", basePrice);
      
      // Khởi tạo lại lưới với giá cơ sở mới
      InitializeGridLevels();
      
      Print("Đã khởi tạo lại lưới với giá cơ sở mới");
      Print("Đang đặt toàn bộ lưới theo cài đặt...");
      Print("========================================");
      ...
      rsiGridInitialized = true;
      Print("✓ Đã khởi tạo toàn bộ lưới - EA sẽ hoạt động bình thường");
   }
}
...
double GetRSIValue()
{
   ...
   // Lấy giá trị RSI từ buffer (chỉ lấy 1 giá trị - giá trị hiện tại)
   if(CopyBuffer(rsiHandle, 0, 0, 1, rsiBuffer) <= 0)
   {
      Print("Lỗi lấy giá trị RSI: ", GetLastError());
      return -1;
   }
...
void ActivateTradingStop(bool isBuyDirection = true)
{
   ...
   // 2. Tính tổng lệnh BUY và SELL để log
   ...
   // 3. Tìm lệnh dương gần giá nhất TRONG HƯỚNG ĐƯỢC CHỌN và tính điểm A
   double currentPrice = SymbolInfoDouble(_Symbol, SYMBOL_BID);
   double nearestProfitPrice = 0.0;
   double minDistance = DBL_MAX;
   bool foundProfitPosition = false;
   
   Print("=== XÓA TP CỦA TẤT CẢ LỆNH (CẢ BUY VÀ SELL) ===");
   ...
   // 3a. XÓA TP CỦA TẤT CẢ LỆNH (CẢ BUY VÀ SELL, CẢ DƯƠNG VÀ ÂM)
   Print("--- Bước 1: Xóa TP của TẤT CẢ lệnh ---");
   for(int i = 0; i < PositionsTotal(); i++)
   {
      ulong ticket = PositionGetTicket(i);
      if(ticket > 0)
      {
            ...
            PositionGetString(POSITION_SYMBOL) == _Symbol)
         {
            ...
            // Xóa TP của TẤT CẢ lệnh (cả Buy và Sell, cả dương và âm)
            if(currentTP > 0)
            {
               // Xóa TP (đặt TP = 0), giữ nguyên SL
               ...
               {
                  Print("  ✓ Đã xóa TP cho lệnh ", (isBuy ? "BUY" : "SELL"), " Ticket ", ticket, " (TP cũ: ", currentTP, ")");
               }
            }
            else
            {
               Print("  - Lệnh ", (isBuy ? "BUY" : "SELL"), " Ticket ", ticket, " không có TP (TP = 0), bỏ qua");
            }
         }
      }
   }
   
   // 3b. Tìm lệnh dương gần giá nhất TRONG HƯỚNG ĐƯỢC CHỌN để tính điểm A
   Print("--- Bước 2: Tìm lệnh DƯƠNG gần giá hiện tại nhất TRONG HƯỚNG ", (isBuyDirection ? "BUY" : "SELL"), " ---");
